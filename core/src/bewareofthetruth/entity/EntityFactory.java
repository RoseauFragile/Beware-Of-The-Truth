...
import bewareofthetruth.entity.components.player.PlayerGraphicsComponent;
import bewareofthetruth.entity.components.player.PlayerInputComponent;
...
public class EntityFactory {
	...
	private EntityFactory(){
		...
		final Array<EntityConfig> townFolkConfigs = Entity.getEntityConfigs(TOWN_FOLK_CONFIGS);
		for( final EntityConfig config: townFolkConfigs){
			_entities.put(config.getEntityID(), config);
		}

		final Array<EntityConfig> environmentalEntityConfigs = Entity.getEntityConfigs(ENVIRONMENTAL_ENTITY_CONFIGS);
		for( final EntityConfig config: environmentalEntityConfigs){
			_entities.put(config.getEntityID(), config);
		}
	...
	public Entity getEntityByName(EntityName entityName){
		final EntityConfig config = new EntityConfig(_entities.get(entityName.toString()));
		final Entity entity = Entity.initEntity(config);
		return entity;
	}
